...
# Custom matcher for account paths that repolaces the account_id with a static value.
class IgnoreAccountMatcher
  ...
  def call
    ...
    right_uri = strip_uri(left_request.uri)
  ...
  def strip_uri(uri)
    ...
    uri.path = uri.path.gsub(%r{accounts/\w+/}, "accounts/test_account_id/")
